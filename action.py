...
# def words(text):
# extract individual words (only alphabets) and into lowercase
# eg; 'The' same as 'the', "dont't" will be seen as two words "don" and 't' 
def words(text): return re.findall('[a-z]+', text.lower())

# def train(features):
# will train Probability Model
...
# NWORDS is big collection of known words formed by reading another files and keeping information how many times each word had appeared in the doccument
...
# deletes:
# returns list of edits with deletion edit
# deletion edit (remove one letter)
# when len(word) is n there are n elements in this list

# transposition:
# returns list of edits with transposition edits
# transposition edit (swap adjacent letters)
# when len(word) is n there are n-1 elements in this list

# alterations:
# returns list of edits with alteration edits
# alteration edit (change one letter to another)
# when len(word) is n there are 26n elements in this list

# inserts:
# returns list of edits with insertion edits
...
# def correct(word):
# Spell Correct Logic according to norvig:
